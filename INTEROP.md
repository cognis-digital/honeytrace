...
How **honeytrace** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `honeytrace` lives in the **General OSINT / security tool** cluster.
...
| from | relation | to |
|---|---|---|
| `honeytrace` | JSON findings compose with | [`awesome-cognis`](https://github.com/cognis-digital/awesome-cognis) |
| `honeytrace` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `honeytrace` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `honeytrace` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the general osint / security tool cluster.** `honeytrace` output feeds the next tool:
```bash
honeytrace ... --format json > out.json          # this tool's findings (see `honeytrace --help`)
...
honeytrace ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
honeytrace ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
honeytrace ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `honeytrace`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `honeytrace` slots
