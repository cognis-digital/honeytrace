"""Native cognis-connect emit for honeytrace — forward findings to any platform.

Maps honeytrace's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    honeytrace ... --format json | honeytrace-emit --to stix
    honeytrace-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    """Tailor a honeytrace record toward the Finding contract before normalize().
